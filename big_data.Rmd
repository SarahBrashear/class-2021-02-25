...
Create a 1% sample of this data, called 'small'.

make a bar plot of dmonth using 'small'

```{r}
small <- x %>%
  slice_sample(n = 74194) 
```
...
small %>%
  ...
  pivot_wider(names_from = dmonth,
              names_prefix = "month",
              values_from = deaths) %>%
  mutate(death_ratio = month1 / month7)
```

```{r}
small %>%
  ggplot(mapping = aes(x = death_ratio,
                       y = dyear)) +
  geom_point() +
